...
data FuseOperations fh = FuseOperations
      { -- | Implements 'System.Posix.Files.getSymbolicLinkStatus' operation
        ...
        --   flags.
        fuseOpen :: B.ByteString -> OpenMode -> OpenFileFlags -> IO (Either Errno fh),
        ...
        fuseRead :: B.ByteString -> fh -> ByteCount -> FileOffset
        ...
        fuseWrite :: B.ByteString -> fh -> B.ByteString -> FileOffset
                  -> IO (Either Errno ByteCount),

        -- | Implements @statfs(2)@.
        fuseGetFileSystemStats :: B.ByteString -> IO (Either Errno FileSystemStats),
        ...
        fuseFlush :: B.ByteString -> fh -> IO Errno,
        ...
        -- file.
        fuseRelease :: B.ByteString -> fh -> IO (),

        -- | Implements @fsync(2)@.
        fuseSynchronizeFile :: B.ByteString -> SyncType -> IO Errno,

        -- | Implements @opendir(3)@.  This method should check if the open
        --   operation is permitted for this directory.
        fuseOpenDirectory :: B.ByteString -> IO Errno,
        ...
        fuseReadDirectory :: B.ByteString -> IO (Either Errno [(B.ByteString, FileStat)]),

        -- | Implements @closedir(3)@.
        fuseReleaseDirectory :: B.ByteString -> IO Errno,

        -- | Synchronize the directory's contents; analogous to
        --   'fuseSynchronizeFile'.
        fuseSynchronizeDirectory :: B.ByteString -> SyncType -> IO Errno,
        ...
        fuseAccess :: B.ByteString -> Int -> IO Errno, -- FIXME present a nicer type to Haskell
...
withStructFuse pFuseChan pArgs ops handler f =
    ...
    where fuseHandler :: e -> IO CInt
          ...
          wrapGetAttr pFilePath pStat = handle fuseHandler $
              do filePath <- B.packCString pFilePath
          ...
          wrapSymLink pSource pDestination = handle fuseHandler $
              do source <- B.packCString pSource
                 destination <- B.packCString pDestination
                 (Errno errno) <- (fuseCreateSymbolicLink ops) source destination
                 return (- errno)
          wrapRename :: CRename
          wrapRename pOld pNew = handle fuseHandler $
              do old <- B.packCString pOld
                 new <- B.packCString pNew
...
--                 flags <- (#peek struct fuse_file_info, flags) pFuseFileInfo
          ...
          wrapReadDir pFilePath pBuf pFillDir off pFuseFileInfo =
            handle fuseHandler $ do
              filePath <- B.packCString pFilePath
              let fillDir = mkFillDir pFillDir
              let filler :: (B.ByteString, FileStat) -> IO ()
                  filler (fileName, fileStat) =
                    B.useAsCString fileName $ \ pFileName ->
...
pokeCStringLen :: CStringLen -> B.ByteString -> IO ()
pokeCStringLen (pBuf, bufSize) src = bsToBuf pBuf src bufSize --B.useAsCStringLen src $ \(pSrcBuf, srcSize) ->
    --pokeArray pBuf $ take bufSize $ map castCharToCChar src
    --copyArray pBuf pSrcBuf (srcSize + 1)

pokeCStringLen0 :: CStringLen -> B.ByteString -> IO ()
pokeCStringLen0 = pokeCStringLen --(pBuf, bufSize) src =
